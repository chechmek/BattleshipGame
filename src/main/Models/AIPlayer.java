...
import main.Abstract.PlayerBase;
...
import static main.Models.Map.breakAroundShip;
...
public class AIPlayer extends PlayerBase
{
    ...
    public boolean isValidShip(Coordinate coord1, Coordinate coord2, int length, Cell[][] localGrid) {
        if (coord1.X == coord2.X && coord2.Y - coord1.Y == length - 1) {
            for(int i = 0; i < length; i++){
                if(localGrid[coord1.X][coord1.Y + i].withShip() || localGrid[coord1.X][coord1.Y + i].getState() == new StateUnavailable())
                    return false;
            }
            return true;
        } else if (coord1.Y == coord2.Y && coord2.X - coord1.X == length - 1) {
            for(int i = 0; i < length; i++){
                if(localGrid[coord1.X + i][coord1.Y].withShip() || localGrid[coord1.X + i][coord1.Y].getState() == new StateUnavailable())
                    return false;
            }
            return true;
        }
    ...
    public Ship setCarrierShip(Scanner input, Cell[][] localGrid) throws Exception {
        ...
        while (!isCreated) {
            ...
            try {
                ...
                isCreated = true;
                changeLocalGrid(ship, localGrid, new StateCarrier());
                return ship;
    ...
    public Ship setSubmarineShip(Scanner input, Cell[][] localGrid) throws Exception {
        boolean isCreated = false;
        Submarine ship;
        while (!isCreated) {
            ...
            try {
                ...
                isCreated = true;
                changeLocalGrid(ship, localGrid, new StateSubmarine());
                return ship;
    ...
    public Ship setBattleShip(Scanner input, Cell[][] localGrid) throws Exception {
        boolean isCreated = false;
        Battleship ship;
        while (!isCreated) {
            for(int j=0;j<Battleship.Count;j++)
            {
                ...
                try {
                    ...
                    isCreated = true;
                    changeLocalGrid(ship, localGrid, new StateBattleship());
                    return ship;
    ...
    public Ship setPatrolShip(Scanner input, Cell[][] localGrid) throws Exception {
        boolean isCreated = false;
        Patrol ship;
        while (!isCreated) {
            for(int j=0;j<Patrol.Count;j++)
            {
                ...
                try {
                    ...
                    isCreated = true;
                    changeLocalGrid(ship, localGrid, new StatePatrol());
                    return ship;
